...
Karst — приложение для подключения к VLESS-серверам и подпискам на Windows. Добавляешь ссылку/подписку, выбираешь сервер из списка и подключаешься через системный TUN-туннель на sing-box. Подписки обновляются автоматически, есть проверка задержки серверов и три режима маршрутизации. Тёмная и светлая тема, при первом запуске подстраивается под системную, далее можно изменить в настройках.
