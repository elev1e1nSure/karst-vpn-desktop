...
Пер-серверный override (колонка `core` в таблице `servers`) — отложен до явного запроса.
...
Открытый вопрос до старта: глобальный выбор ядра или пер-серверный. Влияет на схему БД.
